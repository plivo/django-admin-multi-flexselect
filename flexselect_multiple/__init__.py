...
try: FLEXSELECT_MULTIPLE.update(settings.FLEXSELECT_MULTIPLE)
...
class FlexSelectMultipleWidget(SelectMultiple):
    instances = {}
    """ Instances of widgets with their hashed names as keys."""

    class Media:
        js = []
        if FLEXSELECT_MULTIPLE['include_jquery']:
    ...
    def __init__(self, base_field, modeladmin, request, *args,
        ...
        FlexSelectMultipleWidget.instances[self.hashed_name] = self
    ...
    def _build_js(self):
        ...
        return """\
            <script>
                ...
                flexselect_multiple.fields = flexselect_multiple.fields || {};
                flexselect_multiple.fields.%s = %s;
            </script>""" % (
    ...
    def render(self, name, value, attrs=None, choices=(), *args, **kwargs):
        ...
        html = []
        html.append(super(FlexSelectMultipleWidget, self).render(
            name, value, attrs=attrs,
            *args, **kwargs
        ))
        html.append(self._build_js())
        html.append('<span class="flexselect_multiple_details">')
